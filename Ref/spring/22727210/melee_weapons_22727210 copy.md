...
- Notable high points from base-damage values in the attackset rows:
  - `Attackset_Baton_Empty` (uncharged/drained; `PlayerWeapon_Empty`) light slots: `4.0`; heavy slots: `14.0`; directional poke: `2.0`; directional overhead: `5.0`.
...
## Weapon Timing Fields (Windup/Threshold/Delay)

Yes: weapon attacksets carry the same timing control fields as clan attacksets, per combo slot (`Attacks[i]`):

- `MinimumWindup`
- `MaximumWindup`
- `HeavyThreshold`
- `ThresholdBias`
- `LightAttack.BufferDelay` and `LightAttack.ComboDelay`
- `HeavyAttack.BufferDelay` and `HeavyAttack.ComboDelay`

Source path:
- `EXPORTS/FModel/22727210/Exports/Bloodlines2/Content/WrestlerCommon/Pawns/PlayerCharacter/Weapons/Attackset_*.json`

### Per-attackset timing values (across all combo slots)

Values below are unique-value ranges seen across each attackset's slots, not just slot 0.

| Attackset | MinWU | MaxWU | HeavyThresh | ThresholdBias | Light ComboDelay | Light BufferDelay | Heavy ComboDelay | Heavy BufferDelay |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `Attackset_Bat` | 0.24 | 1.1 | 0.7 | 0.05 | 0.3 / 0.7 | 0.01 / 0.3 | 1.0 | 0.3 / 0.5 |
| `Attackset_Baton_Empty` | 0.24 | 1.1 | 0.7 | 0.05 | 0.3 / 0.7 | 0.01 / 0.3 | 0.7 / 0.8 | 0.3 / 0.5 |
| `Attackset_Baton_Loaded` | 0.24 | 1.1 | 0.7 | 0.05 | 0.3 / 0.7 | 0.01 / 0.3 | 0.7 / 0.8 | 0.3 / 0.5 |
| `Attackset_Knife` | 0.03 / 0.06 | 1.0 | 0.5 | 0.2 | 0.25 / 0.35 | 0.01 | 0.8 | 0.5 |
| `Attackset_Machete` | 0.06 / 0.2 | 1.0 | 0.7 | 0.2 | 0.2 / 0.3 / 0.6 | 0.01 / 0.2 | 0.7 / 0.8 | 0.5 |
| `Attackset_SledgeHammer` | 0.4 | 1.7 | 1.0 | 0.05 | 0.4 | 0.1 | 1.4 | 0.5 |
| `Attackset_SpikeBat` | 0.3 | 1.1 | 0.7 | 0.05 | 0.3 / 0.7 | 0.01 / 0.3 | 0.7 / 0.8 | 0.3 / 0.5 |
| `Attackset_Sword` | 0.2 / 0.22 / 0.25 / 0.3 | 1.1 | 0.7 | 0.05 | 0.3 / 0.7 | 0.01 / 0.3 | 0.7 / 0.8 | 0.3 / 0.5 |
| `Attackset_WarHammer` | 0.55 | 1.7 | 1.0 | 0.05 | 0.4 | 0.1 | 1.4 | 0.5 |

### Directional entries have separate timing

Directional attacks (`Directionals[]`) include their own windup and delays (`Windup`, `Attack.ComboDelay`, `Attack.BufferDelay`).
This means directional weapon attacks can run at different cadence than the base Light/Heavy lane in the same attackset.

### Practical cadence model (weapons)

Use the same approximation model as clan combos:

- Light-chain step time `~ MinimumWindup + Light ComboDelay`
- Heavy-chain step time `~ (HeavyThreshold * MaximumWindup) + Heavy ComboDelay`

These are the timing values that determine practical tap/hold pace; full montage sequence length is still mostly visual if chaining continues.